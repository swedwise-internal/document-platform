<script lang="ts">
  ...
  // Detect embedded mode (opened from document platform)
  let isEmbedded = $state(false);

  const handleSaveToDocument = () => {
    const state = get(inputStateStore);
    window.parent.postMessage({ type: 'mermaid-save', code: state.code }, '*');
  };

  const handleCloseEditor = () => {
    window.parent.postMessage({ type: 'mermaid-close' }, '*');
  };
  ...
  onMount(async () => {
    isEmbedded = new URLSearchParams(window.location.search).has('embedded');
    await initHandler();
...
<div class="flex h-full flex-col overflow-hidden">
  ...
  <Navbar mobileToggle={isMobile ? mobileToggle : undefined}>
    ...
    {#if isEmbedded}
      ...
      <Button variant="accent" size="sm" onclick={handleSaveToDocument}>
        Save to document
      </Button>
    {:else}
      <Share />
