...
export default async function IMSTrainingPage() {
  let trainingDocs: Awaited<ReturnType<typeof listDocuments>> = [];

  try {
    trainingDocs = await listDocuments('training', 'ims');
  } catch (error) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-slate-900">IMS Training</h1>
        <p className="mt-1 text-slate-600">
          Training courses and awareness materials for the Integrated Management System
        </p>
      </div>
