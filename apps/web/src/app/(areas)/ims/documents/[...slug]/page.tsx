...
export default async function IMSDocumentPage({ params }: PageProps) {
  const { slug } = await params;
  const slugPath = slug.join('--');
  const document = await loadDocumentBySlug(slugPath, 'ims');
  ...
  const contentFilePath = `ims/${docPath}`;
  ...
  return (
    <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-6 flex items-center justify-between">
        <Link href="/ims/documents" className="text-blue-600 hover:text-blue-800 text-sm">
          ← Back to IMS Documents
        </Link>
        <div className="flex items-center gap-2">
          <EditDocumentButton area="ims" slug={slugPath} />
          <PdfExportButton
            slug={slugPath}
            area="ims"
